...
wave_ended_timer = pygame.time.get_ticks()
enemy_removing = pygame.time.get_ticks()
enemy_spawn_delay = 1000
enemy_dash_delay = 2000
wave_ended = True
wave_delay = 5000
enemy_spawned = True
...
flag_planet_cleared = False
antoher_add_flag = False
another_another_add_flag = False
game_is_stopped = False



GLOBAL_PLANET_DF = {1:[2, 1, 4, 0, 6, 2], 2:[2, 2, 0, 12, 8, 0], 3:[2, 2, 0, 10, 8, 0], 4:[3, 1, 0, 0, 10, 10], 5:[3, 2, 0, 0, 2, 20], 6:[4, 2, 10, 5, 0, 0], 7:[4, 1, 0, 0, 10, 12], 8:[3, 2, 0, 0, 7, 8], 9:[1, 1, 0, 0, 0, 15], 10:[5, 2, 0, 0, 23, 0], 11:[5, 4, 0, 0, 12, 2], 12:[6, 2, 0, 0, 2, 5], 13:[6, 3, 10, 12, 0, 0], 14:[6, 4, 2, 18, 7, 0], 15:[7, 3, 5, 5, 15, 0], 16:[7, 5, 0, 0, 0, 10], 17:[8, 4, 5, 5, 12, 4], 18:[8, 5, 4, 20, 0, 0], 19:[7, 4, 0, 18, 0, 0], 20:[10, 2, 0, 0, 13, 6], 21:[12, 6, 0, 0, 14, 7], 22:[10, 2, 6, 8, 4, 4], 23:[12, 5, 0, 10, 0, 4], 24:[12, 8, 5, 5, 5, 5], 25:[15, 5, 12, 14, 4, 5]}



food_health_on_diff = [[12, 60, 95, 52],[12, 60, 95, 52],[12, 60, 95, 52], [10, 57, 90, 48], [6, 30, 47,  26]]
toxic_planets = [6,8, 14, 17]
cold_planets = [7, 15]
demon_planets = [9, 22, 23,20,18]
guns_that_dont_need_cooldown = [8-1, 9-1, 13-1, 18-1, 20-1, 34-1, 37-1, 39-1, 41-1]
...
font_6 =  pygame.font.Font("project-VAK/img/gui/Stacked pixel.ttf", 15)
font_7 =  pygame.font.Font("project-VAK/img/gui/Stacked pixel.ttf", 1000)
font_8 =  pygame.font.Font("project-VAK/img/gui/game-over.ttf", 140)
font_9 = pygame.font.Font("project-VAK/img/gui/ThaleahFat.ttf", 128)
...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y, scale, speed, health, ammo, reload_time, grenades=5, grenades_elect=3, animations=None, shield_have_it_or_not=False,path_to_sprites=None):
        pygame.sprite.Sprite.__init__(self)
        self.second_breath_perk_used = False
        self.is_alive = True
        self.speed = speed
        self.ammo = ammo
        ...
        self.move_last_call = pygame.time.get_ticks()
        self.dash_animation = 10
        self.dash_anim = 20
        ...
        self.wave = 0
        self.horror_cooldown = 0
        self.was_given_s = shield_have_it_or_not
        if shield_have_it_or_not:
            self.shield = 100
        else:
            self.shield = -1
        if animations == None:
            ...
            for animation in animation_types:
                temp_list = []
                num_of_frames = len(os.listdir(f'{path_to_sprites}/{animation}'))
                for i in range(num_of_frames):
                    img = pygame.image.load(f'{path_to_sprites}/{animation}/{i+1}.png').convert_alpha()
                    img = pygame.transform.scale(img, (int(img.get_width() * scale), int(img.get_height() * scale)))
                    temp_list.append(img)
                self.animation_list.append(temp_list)
            temp_list = []
            num_of_frames = len(os.listdir(f'project-VAK/img/enemys/death'))
            for i in range(num_of_frames):
                img = pygame.image.load(f'project-VAK/img/enemys/death/{i+1}.png').convert_alpha()
                img = pygame.transform.scale(img, (int(img.get_width() * scale), int(img.get_height() * scale)))
                temp_list.append(img)
            self.animation_list.append(temp_list)
    ...
    def check_states(self):
        ...
        if skill_player_have[10] == 1:
            self.reload_time = 40
            rel_sound = pygame.mixer.Sound("project-VAK/img/sfx/game/rel.wav")
            sounds_group.append(rel_sound)
        else:
            rel_sound = pygame.mixer.Sound("project-VAK/img/sfx/game/rel_s.wav")
            sounds_group.append(rel_sound)
            self.reload_time = 80
        if skill_player_have[6] == 1:
            self.was_given_s = True
    ...
    def shoot(self):
        if self.shoot_cooldown == 0 and self.ammo > 0:
            ...
            if guns_player_have[21-1] == 2:
                self.shoot_cooldown = 20
                angle = get_angle_to_cursor(self.rect)
                bullet = Bullet(self.rect.centerx, self.rect.centery, angle, "player",13, 200, 1)
                bullet_group.add(bullet)
            ...
            if guns_player_have[24-1] == 2:
                ...
                angle = get_angle_to_cursor(self.rect)
                if bullet_cnt == 1:
                    bullet = Bullet(self.rect.centerx, self.rect.centery, angle, "player",speed_pos, poss_damage, 1)
                    bullet_group.add(bullet)
                    self.ammo -= 1
                if bullet_cnt == 2:
                    bullet = Bullet(self.rect.centerx, self.rect.centery, angle, "player",speed_pos, poss_damage, 1)
                    bullet_group.add(bullet)
                    bullet1 = Bullet(self.rect.centerx, self.rect.centery, angle, "player",speed_pos - 1, poss_damage, 1)
                    bullet_group.add(bullet1)
                    self.ammo -= 2
                if bullet_cnt == 5:
                    bullet = Bullet(self.rect.centerx, self.rect.centery, angle, "player",speed_pos, poss_damage, 1)
                    ...
                    bullet1 = Bullet(self.rect.centerx, self.rect.centery, angle, "player",speed_pos - 1, poss_damage, 1)
                    bullet_group.add(bullet1)
                    bullet2 = Bullet(self.rect.centerx, self.rect.centery, angle, "player",speed_pos - 2, poss_damage, 1)
                    bullet_group.add(bullet2)
                    bullet3 = Bullet(self.rect.centerx, self.rect.centery, angle, "player",speed_pos - 3, poss_damage, 1)
                    bullet_group.add(bullet3)
                    bullet4 = Bullet(self.rect.centerx, self.rect.centery, angle, "player",speed_pos - 4, poss_damage, 1)
            ...
            if guns_player_have[35-1] == 2:
                self.shoot_cooldown = 20
                angle = get_angle_to_cursor(self.rect)
                bullet = Bullet(self.rect.centerx, self.rect.centery, angle, "player",8, 700, 3)
                bullet_group.add(bullet)
                self.ammo -= 1
            if guns_player_have[36-1] == 2:
                self.shoot_cooldown = 7
                angle = get_angle_to_cursor(self.rect)
                bullet = Bullet(self.rect.centerx, self.rect.centery, angle, "player",22, 200, 1)
                bullet_group.add(bullet)
                self.ammo -= 1
            if guns_player_have[37-1] == 2:
                self.shoot_cooldown = 0
                angle = get_angle_to_cursor(self.rect)
                bullet = Bullet(self.rect.centerx, self.rect.centery, angle, "player",18, 300, 1)
                bullet1 = Bullet(self.rect.centerx, self.rect.centery, angle - 180, "player",18, 300, 1)
                bullet_group.add(bullet)
                bullet_group.add(bullet1)
                self.ammo -= 2
            if guns_player_have[38-1] == 2:
                ...
                angle = get_angle_to_cursor(self.rect)
                bullet = Bullet(self.rect.centerx, self.rect.centery, angle + 40, "player",22, 200, 1)
                bullet1 = Bullet(self.rect.centerx, self.rect.centery, angle + 80, "player",22, 200, 1)
                bullet2 = Bullet(self.rect.centerx, self.rect.centery, angle + 120, "player",22, 200, 1)
                bullet3 = Bullet(self.rect.centerx, self.rect.centery, angle + 160, "player",22, 200, 1)
                bullet4 = Bullet(self.rect.centerx, self.rect.centery, angle, "player",22, 200, 1)
                bullet5 = Bullet(self.rect.centerx, self.rect.centery, angle - 40, "player",22, 200, 1)
                bullet6 = Bullet(self.rect.centerx, self.rect.centery, angle - 80, "player",22, 200, 1)
                bullet7 = Bullet(self.rect.centerx, self.rect.centery, angle - 120, "player",22, 200, 1)
                bullet8 = Bullet(self.rect.centerx, self.rect.centery, angle - 160, "player",22, 200, 1)
                bullet9 = Bullet(self.rect.centerx, self.rect.centery, -angle, "player",22, 200, 1)
                bullet_group.add(bullet)
                bullet_group.add(bullet1)
                bullet_group.add(bullet2)
                bullet_group.add(bullet3)
            ...
            if guns_player_have[39-1] == 2:
                self.shoot_cooldown = 0
                angle = get_angle_to_cursor(self.rect)
                bullet = Bullet(self.rect.centerx, self.rect.centery, angle, "player",44, 100, 1)
                bullet_group.add(bullet)
                self.ammo -= 1
            if guns_player_have[40-1] == 2:
                self.shoot_cooldown = 12
                angle = get_angle_to_cursor(self.rect)
                bullet = Bullet(self.rect.centerx, self.rect.centery, angle + 5, "player",17, 400, 2)
                bullet1 = Bullet(self.rect.centerx, self.rect.centery, angle + 10, "player",17, 400, 2)
                bullet2 = Bullet(self.rect.centerx, self.rect.centery, angle - 5, "player",17, 400, 2)
                bullet3 = Bullet(self.rect.centerx, self.rect.centery, angle - 10, "player",17, 400, 2)
                bullet4 = Bullet(self.rect.centerx, self.rect.centery, angle + 1, "player",17, 400, 2)
                bullet5 = Bullet(self.rect.centerx, self.rect.centery, angle - 1, "player",17, 400, 2)
                bullet_group.add(bullet)
                bullet_group.add(bullet1)
                bullet_group.add(bullet2)
                bullet_group.add(bullet3)
            ...
            if guns_player_have[41-1] == 2:
                self.shoot_cooldown = 0
                angle = get_angle_to_cursor(self.rect)
                bullet = Bullet(self.rect.centerx, self.rect.centery, angle, "player",25, 500, 4)
                bullet_group.add(bullet)
                self.ammo -= 1
    def dash(self):
        now = pygame.time.get_ticks()
        if now - self.move_last_call >= self.dash_animation:
            self.move_last_call = now
            self.move(move_right, move_left, move_up, move_down)
            self.dash_anim += 1
    ...
    def update(self):
        ...
        self.check_alive()
        self.hitbox.center = (self.rect.centerx + 2, self.rect.centery + 5)
        if skill_player_have[11] == 1 and self.second_breath_perk_used is False and current_window_selected=="play on planet":
            if self.wave == 5:
                self.speed += 1
                if self.was_given_s:
                    self.shield = 100
                self.health = self.max_health
                self.second_breath_perk_used = True
        ...
        if self.horror_cooldown != 0:
            if self.is_alive and (diff_ch == 5 or diff_ch == 10):
                self.horror_cooldown -= 1
        else:
            if self.is_alive and (diff_ch == 5 or diff_ch == 10):
                self.horror_cooldown = 1000
                player.health -= 1
...
def get_enemy_quantity(f_c, s_c, r, d, s, sc):
    g = int(f_c*1.01305724**((planet_which_you_ch-1)*s_c + player.wave) / 5)
    enemy_quantity = {"Rush": r, "Dash": d, "Shooter": s, "Scattered": sc}
    keys = list(enemy_quantity.keys())
    remaining = g - sum(enemy_quantity.values())

    while remaining > 0:
        key = random.choice(keys)
        enemy_quantity[key] += 1
        remaining -= 1
...
class Bullet(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.sender == "enemy":
            self.rect.x += self.speed * math.cos(math.radians(self.angle)) * 4
            self.rect.y += self.speed * math.sin(math.radians(self.angle)) * 4
        else:
        ...
        if player.hitbox.colliderect(self.rect) and self.sender != "player":
            if player.is_alive:
                if skill_player_have[14] == 1 and (player.wave == 1 or player.wave == 2) and player.health - self.damage <= 1:
                    player.health = 1
                else:
                    if player.shield > 0:
        ...
        for enemy in enemies_group:
            if pygame.sprite.collide_rect(enemy, self) and self.sender != "enemy":
                if enemy.is_alive:
                    if skill_player_have[7] == 1 and player.health <= 20:
                        mod_killing = 2
                    else:
                        mod_killing = 1
                    if guns_player_have[-6] == 2 and planet_which_you_ch in toxic_planets:
                        mod_kill_for_spec_gun = 3
                    else:
                        mod_kill_for_spec_gun = 1
                    if skill_player_have[8] == 1 and planet_which_you_ch in demon_planets:
                            enemy.health -= self.damage * 2 * mod_killing * mod_kill_for_spec_gun
                    else:
                        enemy.health -= self.damage * mod_killing * mod_kill_for_spec_gun
                    if guns_player_have[41-1] != 2:
                        self.kill()
class Enemy(Player):
    def __init__(self, enemy_type, necessary_distance, threshold, attack_cooldown=None, melee_damage=None, enemy_dash_cooldown=None, bullet_spec=None, random_bullet_spec=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.enemy_type = enemy_type
        self.necessary_distance = necessary_distance
        self.threshold = threshold
        self.attack_cooldown = attack_cooldown
        self.melee_damage = melee_damage
        self.bullet_spec = bullet_spec
        self.random_bullet_spec = random_bullet_spec
        self.enemy_dash_cooldown = enemy_dash_cooldown
    ...
    def update(self):
        ...
        if self.which == 2:
            ...
            if self.timer <= 0:
                ...
                self.kill()
                for enemy in enemies_group:
                    if abs(self.rect.centerx - enemy.rect.centerx) < 250 and abs(
                            self.rect.centery - enemy.rect.centery) < 250:
                        enemy.health -= 1200
...
Name_v = Animate_smt(25, 8, "gui/name/08_", False, 13, 7)
...
hit_sound = pygame.mixer.Sound("project-VAK/img/sfx/game/hit.wav")
lazer_gun_sound = pygame.mixer.Sound("project-VAK/img/sfx/game/Laser_Gun.wav")
...
over_noise = pygame.mixer.Sound("project-VAK/img/sfx/game/game_over.mp3")
grand_over_noise = pygame.mixer.Sound("project-VAK/img/sfx/game/grand_lose.mp3")
pause_noise = pygame.mixer.Sound("project-VAK/img/sfx/game/Pause.wav")
hit_s = pygame.mixer.Sound("project-VAK/img/sfx/game/hit.wav")
cant_s = pygame.mixer.Sound("project-VAK/img/sfx/game/cant.mp3")
eat_s = pygame.mixer.Sound("project-VAK/img/sfx/game/eat.wav")
death_enemy = pygame.mixer.Sound("project-VAK/img/sfx/game/enemy_death.wav")
happy_s = pygame.mixer.Sound("project-VAK/img/sfx/game/end-good.mp3")
shield_end = pygame.mixer.Sound("project-VAK/img/sfx/game/Thunder.wav")
sbp_s = pygame.mixer.Sound("project-VAK/img/sfx/game/second-brearh.mp3")
sounds_group.append(shield_end)
sounds_group.append(sbp_s)
sounds_group.append(click_sound)
sounds_group.append(happy_s)
sounds_group.append(lazer_gun_sound)
sounds_group.append(enter_galaxy_sound)
sounds_group.append(counter_sound_not_one)
sounds_group.append(hit_sound)
sounds_group.append(counter_sound_one)
sounds_group.append(select_but)
sounds_group.append(city_noise)
sounds_group.append(over_noise)
sounds_group.append(pause_noise)
sounds_group.append(grand_over_noise)
sounds_group.append(hit_s)
sounds_group.append(eat_s)
sounds_group.append(cant_s)
sounds_group.append(death_enemy)
...
if skill_player_have[1] == 1:
    speed_player_constant = 8
else:
    speed_player_constant = 6
...
if skill_player_have[6] == 1:
    constant_shild_have = True
else:
    constant_shild_have = False


player = Player(780, 460, 3, speed_player_constant, health_player_constant, ammo_for_player_constant, reload_player_constant, shield_have_it_or_not=constant_shild_have)
...
GLOBAL_SRPTIE_ENEMY = {1:["project-VAK/img/enemys/first/Nova prime1/slime","project-VAK/img/enemys/first/Nova prime1/fly-eye","project-VAK/img/enemys/first/Nova prime1/bat","project-VAK/img/enemys/first/Nova prime1/ghost"], 2: ["project-VAK/img/enemys/first/Astoria2/crab", "project-VAK/img/enemys/first/Astoria2/crab", "project-VAK/img/enemys/first/Astoria2/lizard","project-VAK/img/enemys/first/Astoria2/lizard"], 3:["project-VAK/img/enemys/first/Olympus3/goblin","project-VAK/img/enemys/first/Olympus3/goblin","project-VAK/img/enemys/first/Olympus3/bird","project-VAK/img/enemys/first/Olympus3/bird"], 4:["project-VAK/img/enemys/first/Nebulous4/mag","project-VAK/img/enemys/first/Nebulous4/mag","project-VAK/img/enemys/first/Nebulous4/mag","project-VAK/img/enemys/first/Nebulous4/mag"], 5:["project-VAK/img/enemys/first/Infinity5/wiz","project-VAK/img/enemys/first/Infinity5/wiz","project-VAK/img/enemys/first/Infinity5/wiz","project-VAK/img/enemys/first/Infinity5/wiz"], 6:["project-VAK/img/enemys/second/Genesis1/slime","project-VAK/img/enemys/second/Genesis1/slime","project-VAK/img/enemys/second/Genesis1/slime","project-VAK/img/enemys/second/Genesis1/slime"], 7:["project-VAK/img/enemys/second/Equinox2/golem","project-VAK/img/enemys/second/Equinox2/golem","project-VAK/img/enemys/second/Equinox2/golem","project-VAK/img/enemys/second/Equinox2/golem"], 8:["project-VAK/img/enemys/second/Orpheus3/slime","project-VAK/img/enemys/second/Orpheus3/slime","project-VAK/img/enemys/second/Orpheus3/slime","project-VAK/img/enemys/second/Orpheus3/slime"],9:["project-VAK/img/enemys/second/Utopia4/demon","project-VAK/img/enemys/second/Utopia4/demon","project-VAK/img/enemys/second/Utopia4/demon","project-VAK/img/enemys/second/Utopia4/demon"], 10:["project-VAK/img/enemys/second/Nexus5/Dino","project-VAK/img/enemys/second/Nexus5/Dino","project-VAK/img/enemys/second/Nexus5/Dino","project-VAK/img/enemys/second/Nexus5/Dino"], 11:["project-VAK/img/enemys/third/Andromeda1/slave","project-VAK/img/enemys/third/Andromeda1/slave","project-VAK/img/enemys/third/Andromeda1/slave","project-VAK/img/enemys/third/Andromeda1/slave"], 12:["project-VAK/img/enemys/third/Epoch2/hz","project-VAK/img/enemys/third/Epoch2/hz","project-VAK/img/enemys/third/Epoch2/hz","project-VAK/img/enemys/third/Epoch2/hz"], 13:["project-VAK/img/enemys/third/Arcadia3/Mutant", "project-VAK/img/enemys/third/Arcadia3/Mutant", "project-VAK/img/enemys/third/Arcadia3/Mutant", "project-VAK/img/enemys/third/Arcadia3/Mutant"], 14:["project-VAK/img/enemys/third/Apollo4/spider","project-VAK/img/enemys/third/Apollo4/spider","project-VAK/img/enemys/third/Apollo4/slime","project-VAK/img/enemys/third/Apollo4/slime"], 15:["project-VAK/img/enemys/third/Polaris5/idler", "project-VAK/img/enemys/third/Polaris5/idler", "project-VAK/img/enemys/third/Polaris5/golem", "project-VAK/img/enemys/third/Polaris5/golem"], 16:["project-VAK/img/enemys/four/Ethereal1/Master","project-VAK/img/enemys/four/Ethereal1/Master","project-VAK/img/enemys/four/Ethereal1/Master","project-VAK/img/enemys/four/Ethereal1/Master","project-VAK/img/enemys/four/Ethereal1/Master"], 17:["project-VAK/img/enemys/four/Soltisice2/dogs","project-VAK/img/enemys/four/Soltisice2/dogs","project-VAK/img/enemys/four/Soltisice2/humans","project-VAK/img/enemys/four/Soltisice2/humans"], 18:["project-VAK/img/enemys/four/Scepter3/thing","project-VAK/img/enemys/four/Scepter3/thing","project-VAK/img/enemys/four/Scepter3/thing","project-VAK/img/enemys/four/Scepter3/thing"], 19:["project-VAK/img/enemys/four/Prisma4/shit","project-VAK/img/enemys/four/Prisma4/shit","project-VAK/img/enemys/four/Prisma4/shit","project-VAK/img/enemys/four/Prisma4/shit"], 20:["project-VAK/img/enemys/four/Fantasia5/ctulhu","project-VAK/img/enemys/four/Fantasia5/ctulhu","project-VAK/img/enemys/four/Fantasia5/ctulhu","project-VAK/img/enemys/four/Fantasia5/ctulhu"],21:["project-VAK/img/enemys/five/New Earth1/soldier","project-VAK/img/enemys/five/New Earth1/soldier","project-VAK/img/enemys/five/New Earth1/soldier","project-VAK/img/enemys/five/New Earth1/Robot"], 22: ["project-VAK/img/enemys/five/Horizon2/hero","project-VAK/img/enemys/five/Horizon2/hero","project-VAK/img/enemys/five/Horizon2/demon","project-VAK/img/enemys/five/Horizon2/skull"], 23:["project-VAK/img/enemys/five/Valhalla3/bob","project-VAK/img/enemys/five/Valhalla3/bob","project-VAK/img/enemys/five/Valhalla3/him","project-VAK/img/enemys/five/Valhalla3/him"],24: ["project-VAK/img/enemys/five/Radiance4/geiber", "project-VAK/img/enemys/five/Radiance4/geiber","project-VAK/img/enemys/five/Radiance4/geiber","project-VAK/img/enemys/five/Radiance4/geiber"], 25:["project-VAK/img/enemys/six/boots","project-VAK/img/enemys/six/boots","project-VAK/img/enemys/six/hand","project-VAK/img/enemys/six/hand"]}

def get_random_cords(enemy_type_n):
    if enemy_type_n == "Shooter" or enemy_type_n == "Scattered":
...
cursor_img = pygame.image.load('project-VAK/img/gui/cursor.png').convert_alpha()
cursor_img = pygame.transform.scale(cursor_img, (int(cursor_img.get_width() * 3), int(cursor_img.get_height() * 3)))
...
while run:
    if current_window_selected == "main_menu":
        ...
        if ship_level == 1:
            ...
            screen.blit(ship_im, (620, y_for_ship_pos + 150))
        if ship_level == 2:
            ship_im = pygame.image.load("project-VAK/img/gui/ships/Ship2.png").convert_alpha()
            ship_im = pygame.transform.scale(ship_im, (int(ship_im.get_width() * 5), int(ship_im.get_height() * 5)))
            screen.blit(ship_im, (480, y_for_ship_pos - 14))     
        if ship_level == 3:
            ship_im = pygame.image.load("project-VAK/img/gui/ships/Ship3.png").convert_alpha()
            ship_im = pygame.transform.scale(ship_im, (int(ship_im.get_width() * 5), int(ship_im.get_height() * 5)))
            screen.blit(ship_im, (480, y_for_ship_pos))
        if ship_level == 4:
            ship_im = pygame.image.load("project-VAK/img/gui/ships/Ship4.png").convert_alpha()
            ship_im = pygame.transform.scale(ship_im, (int(ship_im.get_width() * 5), int(ship_im.get_height() * 5)))
            screen.blit(ship_im, (480, y_for_ship_pos - 22))
        if ship_level == 5:
            ship_im = pygame.image.load("project-VAK/img/gui/ships/Ship5.png").convert_alpha()
            ship_im = pygame.transform.scale(ship_im, (int(ship_im.get_width() * 5), int(ship_im.get_height() * 5)))
            screen.blit(ship_im, (480, y_for_ship_pos - 42))
        if ship_level == 6:
            ship_im = pygame.image.load("project-VAK/img/gui/ships/Ship6.png").convert_alpha()
            ship_im = pygame.transform.scale(ship_im, (int(ship_im.get_width() * 5), int(ship_im.get_height() * 5)))
            screen.blit(ship_im, (480, y_for_ship_pos - 18))

        draw_text("SPACE-V", font_2, (255, 255, 255), 267, 80)
        Name_v.update()
        Name_v.draw(screen, 166, 250)
        draw_text("ver 1.00", font, (140, 95, 222), 1430, 4)
        draw_text("ver 1.00", font, (204, 188, 232), 1430, 2)
        if ship_level != 6:
            ...
            if y_for_ship_pos >= 320:
                floating_ship_speed = -floating_ship_speed
            if y_for_ship_pos <= 300:
                floating_ship_speed = -floating_ship_speed
        start_button = Button(SCREEN_WIDTH // 45, SCREEN_HEIGHT // 2 + 360, start_im, 7, start_im_s, start_im, "project-VAK/img/sfx/main_menu/start_game.mp3")
        option_button = Button(SCREEN_WIDTH // 45 + 400, SCREEN_HEIGHT // 2 + 360, option_im, 7, option_im_s)
        credit_button = Button(SCREEN_WIDTH // 45 + 800, SCREEN_HEIGHT // 2 + 360, credit_im, 7, credit_im_s)
        exit_button = Button(SCREEN_WIDTH // 45 + 1200, SCREEN_HEIGHT // 2 + 360, exit_im, 7, exit_im_s)
    ...
    if current_window_selected == "select planet":
        ...
        if page == 2:
            ...
            if diff_ch != 0:
                if start_play_on_planet.draw(screen):
                    ...
                    possible_damage = [100, 200, 400]
                    bullets_range_p = [1,2,5]
                    speed_pos_c = [12, 9, 20]
                    ammo_cnt_list_pos = [30, 40,20]
                    bullet_cnt = random.choice(bullets_range_p)
                    speed_pos = random.choice(speed_pos_c)
                    poss_damage = random.choice(possible_damage)
                    ammo_cnt_pos = random.choice(ammo_cnt_list_pos)
                    player = Player(780, 460, 3, speed_player_constant, health_player_constant, ammo_for_player_constant, reload_player_constant, shield_have_it_or_not=constant_shild_have)
                    player.check_states()
            ...
            if diff_ch == 4:
                ...
                draw_text("and ready for obstacles. At this difficulty level, food restores 10 % ", font_5, (255,255,255), 350 ,655)
            ...
            if planet_which_you_ch != 25:
                ...
                star_galx1_im = pygame.image.load(f"project-VAK/img/gui/stars/6.png").convert_alpha()
                star_galx1_im = pygame.transform.scale(star_galx1_im, (int(star_galx1_im.get_width() * 3), int(star_galx1_im.get_height() * 3)))
                screen.blit(star_galx1_im, (400 , 310))
                star_galx2_im = pygame.image.load(f"project-VAK/img/gui/stars/7.png").convert_alpha()
                star_galx2_im = pygame.transform.scale(star_galx2_im, (int(star_galx2_im.get_width() * 3), int(star_galx2_im.get_height() * 3)))
                screen.blit(star_galx2_im, (565 , 305))
                star_galx2_im = pygame.image.load(f"project-VAK/img/gui/stars/8.png").convert_alpha()
                star_galx2_im = pygame.transform.scale(star_galx2_im, (int(star_galx2_im.get_width() * 3), int(star_galx2_im.get_height() * 3)))
                screen.blit(star_galx2_im, (750 , 310))
                star_galx2_im = pygame.image.load(f"project-VAK/img/gui/stars/9.png").convert_alpha()
                star_galx2_im = pygame.transform.scale(star_galx2_im, (int(star_galx2_im.get_width() * 3), int(star_galx2_im.get_height() * 3)))
                screen.blit(star_galx2_im, (920 , 286))
                star_galx2_im = pygame.image.load(f"project-VAK/img/gui/stars/10.png").convert_alpha()
                star_galx2_im = pygame.transform.scale(star_galx2_im, (int(star_galx2_im.get_width() * 3), int(star_galx2_im.get_height() * 3)))
                screen.blit(star_galx2_im, (1080 , 269))
        ...
        if page == 33:
            ...
            if skill_player_have[2] != 1:
                draw_text("you cannot go to this planet because you do not have the ", font_4, (255,255,255), 320 , 790)
                draw_text("resistance to toxins", font_4, (55, 191, 17), 550 , 822)
                draw_text("resistance to toxins perk", font_4, (255,255,255), 550 , 820)
                perk = pygame.image.load(f"project-VAK/img/gui/skills/tocsin.png").convert_alpha()
                perk = pygame.transform.scale(perk, (int(perk.get_width() * 1), int(perk.get_height() * 1)))
                screen.blit(perk, (1480 ,799))
            else:
                if start_button.draw(screen):
                    page = 2
        ...
        if page == 34:
            ...
            if skill_player_have[3] != 1:
                draw_text("you cannot go to this planet because you do not have the ", font_4, (255,255,255), 320 , 790)
                draw_text("resistance to cold", font_4, (66, 192, 214), 550 , 822)
                draw_text("resistance to cold perk", font_4, (255,255,255), 550 , 820)
                perk = pygame.image.load(f"project-VAK/img/gui/skills/freeze.png").convert_alpha()
                perk = pygame.transform.scale(perk, (int(perk.get_width() * 1), int(perk.get_height() * 1)))
                screen.blit(perk, (1480 ,799))
            else:
                if start_button.draw(screen):
    ...
    if current_window_selected == "weapons":
        ...
        if page == 1:
            ...
            if b4.draw(screen):
                if guns_player_have[3] == 0:
                    page = 3
                    gun_which = 4
                if guns_player_have[3] == 1 or guns_player_have[3] == 2:
                    guns_player_have[guns_player_have.index(2)] = 1
        ...
        if page == 3:
            ...
            if gun_which == 14:
                ...
                draw_text("New", font_3, (255,255,10), 450, 280)
                draw_text("---------------------------------------------------------------------------------", font_4, (255,255,10), 0, 802)
                draw_text("- 60 charges", font_3, (255,255,255), 450, 350)
    ...
    if current_window_selected == "skill":
        ...
        if page == 2:
            ...
            if skill_which == 7:
                ...
                draw_text("you have a shield with 100 hp", font_4, (255,255,255), 120, 470)
            ...
            if skill_which == 15:
                ...
                draw_text("category: survival rate", font_4, (255,25,25), 320, 220)
                draw_text("With the", font_4, (255,255,255), 120, 350)
                draw_text("'Above the pain'", font_4, (255,25,25), 300, 350)
                draw_text("perk", font_4, (255,255,255), 600, 350)
    ...
    if current_window_selected == "junk":
        ...
        if page == 3:
            ...
            if b1.draw(screen):
                ...
                gun_which = 1
            if b2.draw(screen):
                if guns_player_have[-5] == 1 or guns_player_have[-5] == 2:
                    guns_player_have[guns_player_have.index(2)] = 1
                    guns_player_have[-5] = 2
                page = 5
                gun_which = 2
        ...
        if page == 5:
            ...
            if gun_which == 2:
                ...
                gun = pygame.transform.scale(gun, (int(gun.get_width() * 13), int(gun.get_height() * 13)))
                screen.blit(gun,(0, 250))
                draw_text("shoots a double beam in the direction", font_4, (255,0,90), 470, 140)
                draw_text("of the sight and in the opposite direction", font_4, (255,0,90), 440, 170)
            ...
            if gun_which == 4:
                ...
                draw_text("which can pass through enemies", font_4, (255,0,90), 470, 170)
            ...
            if gun_which == 5:
                ...
                screen.blit(gun,(0, 250))
                draw_text("shoots a wall of 5 bullets", font_4, (255,0,90), 530, 170)
            ...
            if gun_which == 6:
                ...
                draw_text("which passes through enemies", font_4, (255,0,90), 370, 170)
    ...
    if current_window_selected == "play on planet":
        cursor_img = pygame.image.load('project-VAK/img/gui/cursor_shoot.png').convert_alpha()
        cursor_img = pygame.transform.scale(cursor_img, (int(cursor_img.get_width() * 3), int(cursor_img.get_height() * 3)))
        ...
        enemies_group = pygame.sprite.Group(filter(lambda enemy: enemy.is_alive, enemies_group.sprites()))

        for enemy in enemies_group:
            if game_is_stopped is not True:
                if enemy.is_alive:
                    distance_to_player = math.sqrt(
                        (player.rect.x - enemy.rect.x) ** 2 + (player.rect.y - enemy.rect.y) ** 2
                    )

                    if distance_to_player < enemy.necessary_distance:
                        x_direction = player.rect.x - enemy.rect.x
                        y_direction =  player.rect.y - enemy.rect.y

                        move_right = x_direction < 0 and abs(x_direction) > enemy.threshold
                        move_left = x_direction > 0 and abs(x_direction) > enemy.threshold
                        move_up = y_direction > 0 and abs(y_direction) > enemy.threshold
                        move_down = y_direction < 0 and abs(y_direction) > enemy.threshold

                        if move_left or move_right or move_down or move_up:
                            if (move_left and move_right) and not move_up and not move_down:
                                enemy.update_action(0)
                            elif (move_down and move_up) and not move_right and not move_left:
                                enemy.update_action(0)
                            else:
                    ...
                    if enemy.enemy_type == "Rush":
                        enemy.move(move_right, move_left, move_up, move_down)

                        if player.hitbox.colliderect(enemy.rect) and enemy.shoot_cooldown == 0:
                            if skill_player_have[14] == 1 and (player.wave == 1 or player.wave == 2) and player.health - enemy.melee_damage <= 1:
                                player.health = 1
                            else:
                                if player.shield > 0:
                                    player.shield -= enemy.melee_damage
                                else:
                                    player.health -= enemy.melee_damage
                            hit_s.play() 
                            enemy.shoot_cooldown = enemy.attack_cooldown  
            
                    elif enemy.enemy_type == "Dash":
                            if enemy.dash_cooldown == 0:
                                enemy.speed = 10
                                if enemy.dash_anim >= 70:
                                    enemy.dash_anim = 0
                                    enemy.dash_cooldown = enemy.enemy_dash_cooldown
                                enemy.dash()
                            else:
                                enemy.speed = 2
                                enemy.move(move_right, move_left, move_up, move_down)
                        ...
                        if enemy.shoot_cooldown == 0:
                            ...
                            bullet_speed, bullet_damage, bullet_size = enemy.bullet_spec
                            bullet = Bullet(enemy.rect.centerx, enemy.rect.centery, angle, "enemy", bullet_speed, bullet_damage, bullet_size)
                            bullet_group.add(bullet)
                            enemy.shoot_cooldown = enemy.attack_cooldown

                    elif enemy.enemy_type == "Scattered":
                        enemy.move(move_right, move_left, move_up, move_down)
                        ...
                        if enemy.shoot_cooldown == 0:
                            ...
                            num_bullets = 5
                            angle_between_bullets = 10 

                            for i in range(num_bullets):
                                bullet_angle = angle - (angle_between_bullets * (num_bullets - 1) / 2) + (angle_between_bullets * i)
                                bullet_speed, bullet_damage, bullet_size = enemy.bullet_spec
                                bullet = Bullet(enemy.rect.centerx, enemy.rect.centery, bullet_angle, "enemy", bullet_speed, bullet_damage, bullet_size)
                                bullet_group.add(bullet)
        ...
        health_bars = {75: "health_bar4.png", 40: "health_bar3.png", 10: "health_bar2.png", 0: "health_bar1.png",
                    -1: "health_bar0.png"}
        shield_bars = {75: "shield_bar4.png", 40:"shield_bar3.png", 20:"shield_bar2.png", 10:"shield_bar1.png"}
        ammo_bars = {15: "ammo5.png", 10: "ammo4.png", 5: "ammo3.png", 1: "ammo2.png", 0: "ammo1.png", -1: "ammo0.png"}
        shield_bars = {37: "shield_bar4.png", 23: "shield_bar3.png", 10: "shield_bar2.png", 0: "shield_bar1.png"}
        difficulty_modificator = {1: 1, 2: 1.2, 3: 1.5, 4: 2, 5: 2.5}[diff_ch]
        planet_modificator = 1.01 ** planet_which_you_ch
        wave_modificator = 1.1 ** player.wave
        full_modificator = planet_modificator * wave_modificator * difficulty_modificator
        ...
        if player.wave == 0: 
            player.wave = 1
            enemy_quantity = get_enemy_quantity(GLOBAL_PLANET_DF[planet_which_you_ch][0], GLOBAL_PLANET_DF[planet_which_you_ch][1], GLOBAL_PLANET_DF[planet_which_you_ch][2], GLOBAL_PLANET_DF[planet_which_you_ch][3], GLOBAL_PLANET_DF[planet_which_you_ch][4], GLOBAL_PLANET_DF[planet_which_you_ch][5])
        elif player.wave > 0 and player.wave <= 5: 
            enemy_quantity = {key: value for key, value in enemy_quantity.items() if value > 0}
            enemies_to_spawn = sum(enemy_quantity.values())
            if enemy_spawned:
                enemy_spawned = False
                enemy_timer = current_time
            if current_time > 10000 and current_time - enemy_timer >= enemy_spawn_delay and player.is_alive and counters_constat == -1:
                if enemies_to_spawn > 0:
                    enemy_spawned = True
                    random_type = random.choice(list(enemy_quantity.keys()))
                    x,y = get_random_cords(random_type)
                    if planet_which_you_ch >= 6:
                        add_modificator = int(planet_which_you_ch / 2)
                    else:
                        add_modificator = 1
                    if random_type == "Rush":
                        enemy = Enemy(enemy_type=random_type, necessary_distance=30000, threshold=50, melee_damage=int(10*full_modificator), attack_cooldown=int(100//full_modificator), x=x, y=y, scale=3, speed=1*int(full_modificator)+random.randint(1, 2), health=int(30*full_modificator * add_modificator), ammo=0, reload_time=200, animations=['idle','run'], path_to_sprites=GLOBAL_SRPTIE_ENEMY[planet_which_you_ch][0])
                    elif random_type == "Dash":
                        enemy = Enemy(enemy_type=random_type, necessary_distance=30000, threshold=50, melee_damage=int(10*full_modificator), attack_cooldown=int(100//full_modificator), enemy_dash_cooldown=int(500//full_modificator), x=x, y=y, scale=3, speed=int(1*full_modificator)+random.randint(1, 3), health=int(20*full_modificator * add_modificator), ammo=0, reload_time=200, animations=['idle','run'], path_to_sprites=GLOBAL_SRPTIE_ENEMY[planet_which_you_ch][1])
                    elif random_type == "Shooter":
                        enemy = Enemy(enemy_type=random_type, necessary_distance=3000, threshold=500, attack_cooldown=int(200//full_modificator), bullet_spec=[int(2*full_modificator), int(2*full_modificator), 1], x=x, y=y, scale=3, speed=int(2*full_modificator)+random.randint(1, 2), health=int(40*full_modificator * add_modificator), ammo=0, reload_time=200, animations=['idle','run'], path_to_sprites=GLOBAL_SRPTIE_ENEMY[planet_which_you_ch][2])
                    elif random_type == "Scattered":
                        enemy = Enemy(enemy_type=random_type, necessary_distance=30000, threshold=700, attack_cooldown=int(400//full_modificator), bullet_spec=[int(2*full_modificator), int(2*full_modificator), 1], x=x, y=y, scale=3, speed=int(2*full_modificator)+random.randint(1, 2), health=int(50*full_modificator * add_modificator), ammo=0, reload_time=200, animations=['idle','run'], path_to_sprites=GLOBAL_SRPTIE_ENEMY[planet_which_you_ch][3])

                    enemy_quantity[random_type] -= 1
                    ...
                    if len(enemies_group) == 0:
                        if wave_ended:
                            ...
                            wave_ended_timer = current_time
                            bullet_group.empty()
                            enemies_group.empty()
                        if current_time > 10000 and current_time - wave_ended_timer >= wave_delay and player.is_alive:
                            wave_ended = True
                            player.wave += 1
                            counter_sound_one.play()
        ...
        if start_intro == True:
            if intro_fade.fade():
                start_intro = False
                intro_fade.fade_counter = 0
        if counters_constat != -1 and counters_constat != -2:
            counters_constat -= 1
        ...
        if counters_constat <= 160 and counters_constat > 80:
            nujb = pygame.image.load(f"project-VAK/img/gui/num/3.png").convert_alpha()
            nujb = pygame.transform.scale(nujb, (int(nujb.get_width() * 10), int(nujb.get_height() * 10)))
            screen.blit(nujb, (720 , 290))
        if counters_constat <= 80 and counters_constat > 3:
            nujb = pygame.image.load(f"project-VAK/img/gui/num/2.png").convert_alpha()
            nujb = pygame.transform.scale(nujb, (int(nujb.get_width() * 10), int(nujb.get_height() * 10)))
            screen.blit(nujb, (720 , 290))

        if counters_constat == -1:
            if game_is_stopped == False:
                player.update()
                player.draw()
                for enemy in enemies_group:
                    enemy.update()
                ...
                if player.is_alive:
                    if is_shooting:
                        if player.ammo != 0 and player.shoot_cooldown == 0:
                            lazer_gun_sound.stop()
                            lazer_gun_sound.play()
                            
                        player.shoot()
                        if guns_player_have.index(2) not in guns_that_dont_need_cooldown:
                            is_shooting = False
                ...
                border1 = pygame.image.load("project-VAK/img/gui/backstops/1Border.png").convert_alpha()
                border1 = pygame.transform.scale(border1, (int(border1.get_width() * 3), int(border1.get_height() * 3)))
                border2 = pygame.image.load("project-VAK/img/gui/backstops/2Border.png").convert_alpha()
                border2 = pygame.transform.scale(border2, (int(border2.get_width() * 3), int(border2.get_height() * 3)))
                border3 = pygame.image.load("project-VAK/img/gui/backstops/2Border.png").convert_alpha()
                border3 = pygame.transform.scale(border3, (int(border3.get_width() * 3), int(border3.get_height() * 3)))
                
                screen.blit(border1, (910, -2)) 
                screen.blit(border2, (910, 130)) 
                screen.blit(border3, (-440, -2)) 

                if player.wave == 6:
                    wave_text = f'END'
                else:
                    wave_text = f'WAVE {player.wave}'

                draw_text(wave_text, font, (153, 40, 17), 65, 40)
                draw_text(wave_text, font, (255, 255, 255), 65, 37)
                if wave_ended == False and player.wave < 5:
                    draw_text(f"The next wave in {(6000 - (current_time - wave_ended_timer)) // 1000}", font_5, (255, 255, 255), 20, 130)
                
                if planet_cleared:
                    draw_text(f"PLANET CLEARED", font, (153, 40, 17), 600, 300)
                    draw_text(f"PLANET CLEARED", font, (255, 255, 255), 600, 298)
                    draw_text(f"press tab to continue", font, (153, 40, 17), 600, 350)
                    draw_text(f"press tab to continue", font, (255, 255, 255), 600, 348)
                    pygame.mixer_music.stop()
                    if flag_planet_cleared is False:
                        happy_s.play()
                        if planet_which_you_ch == 1:
                            stars_on_planets_gal1[0] = diff_ch
                            week_steel_cnt += random.randint(1, 2) + int(2 * diff_ch)
                            if diff_ch >= 3:
                                steel_cnt += 2
                            if random.randint(1, 10) == 2:
                                black_det_cnt += 2
                            def_det_cnt += random.randint(2, 3) + int(diff_ch)
                            if diff_ch == 1:
                                money += 20
                                money_diamond += 1
                            if diff_ch == 2:
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 2:
                            stars_on_planets_gal1[1] = diff_ch
                            week_steel_cnt += random.randint(2, 3) + int(2 * diff_ch)
                            if diff_ch >= 4:
                                steel_cnt += 3
                            if diff_ch == 3:
                                steel_cnt += 2
                            def_det_cnt += random.randint(2, 3) + int(diff_ch * 2)
                            if diff_ch == 1:
                                money += 20
                                money_diamond += 1
                            if diff_ch == 2:
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 3:
                            stars_on_planets_gal1[2] = diff_ch
                            steel_cnt += 2 * int(diff_ch)
                            if diff_ch >= 3:
                                week_steel_cnt += random.randint(6,8)
                            else:
                                week_steel_cnt += random.randint(3, 4)
                            power_det_cnt += 1
                            if diff_ch == 1:
                                money += 20
                                money_diamond += 0
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 4:
                            stars_on_planets_gal1[3] = diff_ch
                            black_det_cnt += random.randint(0, 1) + int(diff_ch)
                            steel_cnt += random.randint(0,2) + int(2 * diff_ch)
                            week_steel_cnt += random.randint(3, 4)
                            if diff_ch == 1:
                                money += 20
                                money_diamond += 1
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 5:
                            stars_on_planets_gal1[4] = diff_ch
                            if diff_ch >= 4:
                                power_det_cnt += 2
                            else:
                                power_det_cnt += 1
                            week_steel_cnt += random.randint(6,8) + int(diff_ch)
                        if planet_which_you_ch == 6:
                            stars_on_planets_gal2[0] = diff_ch
                            steel_cnt += 2
                            black_det_cnt += random.randint(1,2) * int(2 * diff_ch)
                            def_det_cnt += random.randint(2,3)
                            week_steel_cnt += random.randint(9, 10)
                            toxic_det_cnt += random.randint(1, 3) * int(3 * diff_ch)
                            if diff_ch == 1:
                                money += 30
                                money_diamond += 1
                            if diff_ch == 2:
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 7:
                            stars_on_planets_gal2[1] = diff_ch
                            if skill_player_have[15] == 1:
                                if random.randint(1, 2) == 1:
                                    if diff_ch >= 3:
                                        ...
                                        gold_cnt += 1
                            steel_cnt += random.randint(1, 2)
                            black_det_cnt += random.randint(1, 2) * int(diff_ch)
                            water_det_cnt += random.randint(2, 3)
                            if diff_ch == 1:
                                money += 30
                                money_diamond += 1
                            if diff_ch == 2:
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 8:
                            stars_on_planets_gal2[2] = diff_ch
                            toxic_det_cnt += 4
                            if skill_player_have[15] == 1:
                                if random.randint(1, 2) == 1:
                                    if diff_ch >= 3:
                                        ...
                                        gold_cnt += 1
                                if diff_ch >= 2:
                                    power_det_cnt += random.randint(2, 3)
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 9:
                            stars_on_planets_gal2[3] = diff_ch
                            black_det_cnt += random.randint(1, 3)
                            steel_cnt += random.randint(2, 3) * int(diff_ch)
                            if random.randint(1, 3) == 3:
                                power_det_cnt += int(diff_ch * 2)
                            def_det_cnt += 5
                            if diff_ch == 1:
                                money += 30
                                money_diamond += 0
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 10:
                            stars_on_planets_gal2[4] = diff_ch
                            if skill_player_have[15] == 1:
                                if diff_ch >= 3:
                                    gold_cnt += 3
                                else:
                            ...
                            rare_det_cnt += 1
                        if planet_which_you_ch == 11:
                            stars_on_planets_gal3[0] = diff_ch
                            steel_cnt += random.randint(1, 2)
                            if skill_player_have[15] == 1:
                                gold_cnt += 3
                            else:
                                gold_cnt += 1
                            if diff_ch >= 3:
                                black_det_cnt += random.randint(2,5)
                            week_steel_cnt += 12
                            if diff_ch == 1:
                                money += 40
                                money_diamond += 0
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 12:
                            stars_on_planets_gal3[1] = diff_ch
                            if random.randint(1, 3) == 1:
                                rare_det_cnt += random.randint(1, 2)
                                if diff_ch >= 2:
                                    black_det_cnt += 2
                            black_det_cnt += 2
                            steel_cnt += random.randint(2, 4)
                            if diff_ch >= 2:
                                week_steel_cnt += random.randint(9, 14)
                            else:
                                week_steel_cnt += random.randint(1, 8)
                            if diff_ch == 1:
                                money += 40
                                money_diamond += 0
                            if diff_ch == 2:
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 13:
                            stars_on_planets_gal3[2] = diff_ch
                            black_det_cnt += random.randint(2, 3)
                            if diff_ch >= 2:
                                power_det_cnt += random.randint(1, 2)
                            if diff_ch == 1:
                                money += 40
                                money_diamond += 0
                            if diff_ch == 2:
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 14:
                            stars_on_planets_gal3[3] = diff_ch
                            toxic_det_cnt += random.randint(2, 3)
                            if diff_ch >= 3:
                                if skill_player_have[15] == 1:
                                    gold_cnt += 4
                                else:
                                    gold_cnt += 3
                            steel_cnt += random.randint(3, 5)
                        if planet_which_you_ch == 15:
                            stars_on_planets_gal3[4] = diff_ch
                            water_det_cnt += random.randint(1,3)
                            gold_cnt += random.randint(2, 3)
                            black_det_cnt += 3
                            if diff_ch >= 4:
                                best_det_cnt += 1
                            if diff_ch == 1:
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 16:
                            stars_on_planets_gal4[0] = diff_ch
                            if diff_ch >= 3:
                                power_det_cnt += 1
                            steel_cnt += random.randint(1, 2)
                            if diff_ch >= 3:
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 17:
                            stars_on_planets_gal4[1] = diff_ch
                            power_det_cnt += 1
                            if diff_ch >= 3:
                                toxic_det_cnt += 3
                                if skill_player_have[15] == 1:
                                    better_stl_cnt += random.randint(2, 3)
                            gold_cnt += random.randint(3)
                            black_det_cnt += random.randint(2, 4) * int(diff_ch)
                            toxic_det_cnt += random.randint(2, 4)
                            if diff_ch >= 3:
                                best_det_cnt += 3
                            if diff_ch == 1:
                                money += 60
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 18:
                            stars_on_planets_gal4[2] = diff_ch
                            gold_cnt += 2
                            rare_det_cnt += 4
                            week_steel_cnt += random.randint(10, 14)
                            if diff_ch >= 3:
                                best_det_cnt += 3
                            if diff_ch == 1:
                                money += 70
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 19:
                            stars_on_planets_gal4[3] = diff_ch
                            steel_cnt += random.randint(12,13)
                            if skill_player_have[15] == 1:
                                if random.randint(1, 2) == 2:
                                    gold_cnt += 6
                            if diff_ch >= 4:
                                grass_cnt += random.randint(4, 5)
                            grass_cnt += random.randint(4, 5)    
                            if diff_ch == 1:
                                money += 120
                                money_diamond += 1
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 20:
                            stars_on_planets_gal4[4] = diff_ch
                            if diff_ch >= 3 or skill_player_have[15] == 1:
                                gold_cnt += 7
                            def_det_cnt += random.randint(15,19)
                            if diff_ch >= 2:
                                steel_cnt += random.randint(15,17)
                            if diff_ch == 1:
                                money += 100
                                money_diamond += 2
                            if diff_ch == 2:
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 21:
                            stars_on_planets_gal5[0] = diff_ch
                            if skill_player_have[15] == 1:
                                better_stl_cnt += 7 * int(diff_ch * 2)
                            else:
                                if random.randint(1, 5) == 2:
                                    better_stl_cnt += 5
                            gold_cnt += 5 + (skill_player_have[15] * 6)
                            power_det_cnt += 3
                            best_det_cnt += 4
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 22:
                            stars_on_planets_gal5[1] = diff_ch
                            grass_cnt += random.randint(8, 12)
                            rare_det_cnt += 2
                            steel_cnt += 15
                            if diff_ch == 3:
                                grass_cnt += random.randint(8, 12)
                            ...
                            if diff_ch >= 4:
                                ...
                                best_det_cnt += 2
                        if planet_which_you_ch == 23:
                            stars_on_planets_gal5[2] = diff_ch
                            if skill_player_have[15] == 1:
                                better_stl_cnt += 5
                            gold_cnt += int(diff_ch * 2 * 3.2)
                            if diff_ch == 1:
                                money += 120
                                money_diamond += 2
                            if diff_ch == 2:
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 2
                        if planet_which_you_ch == 24:
                            stars_on_planets_gal5[3] = diff_ch
                            if diff_ch == 2:
                                rare_det_cnt += 5
                                grass_cnt += 13
                            if diff_ch >= 3:
                            ...
                            if diff_ch == 5:
                                ...
                                money_diamond += 1
                        if planet_which_you_ch == 25:
                            stars_on_planets_gal6[0] = diff_ch
                            if diff_ch == 7:
                                money += 500
                                money_diamond += 10
                            if diff_ch == 8:
                            ...
                            if diff_ch == 11:
                                money += 1000
                                money_diamond += 50
                        flag_planet_cleared = True
                        
                draw_text(f'{player.ammo}', font_6, (37, 93, 128), 1110, 35)
                draw_text(f'{player.ammo}', font_6, (255, 255, 255), 1110, 34)
                draw_text(f'AMMO', font, (37, 93, 128), 1133, 40)
                draw_text(f'AMMO', font, (255, 255, 255), 1133, 38)
                draw_status_bar(player.ammo, ammo_bars, 1050, 48)
                if player.shield <= 0 and player.was_given_s and another_another_add_flag is False:
                    shield_end.play()
                    another_another_add_flag = True
                    

                if player.shield <= 0:
                    draw_text(f'{player.health}', font_6, (153, 40, 17), 1378, 35)
                    draw_text(f'{player.health}', font_6, (255, 255, 255), 1378, 34)
                    draw_text(f'HEALTH', font, (153, 40, 17), 1415, 40)
                    draw_text(f'HEALTH', font, (255, 255, 255), 1415, 38)
                    draw_status_bar(player.health, health_bars, 1235, 46)
                if player.shield > 0:
                    draw_text(f'{player.shield}', font_6, (69, 203, 237), 1378, 35)
                    draw_text(f'{player.shield}', font_6, (255, 255, 255), 1378, 34)
                    draw_text(f'shield', font, (69, 203, 237), 1415, 40)
                    draw_text(f'shield', font, (255, 255, 255), 1415, 38)
                    draw_status_bar(player.shield, shield_bars, 1235, 46)
                normal_food = pygame.image.load(f"project-VAK/img/gui/food/normal_food.png").convert_alpha()
                normal_food = pygame.transform.scale(normal_food, (int(normal_food.get_width() * 2), int(normal_food.get_height() * 2)))
                screen.blit(normal_food, (1410, 155))
                draw_text(f"{small_fruit}", font_5, (255,255,255), 1420, 194)
                good_food = pygame.image.load(f"project-VAK/img/gui/food/good_food.png").convert_alpha()
                good_food = pygame.transform.scale(good_food, (int(good_food.get_width() * 2), int(good_food.get_height() * 2)))
                screen.blit(good_food, (1460, 159))
                draw_text(f"{fresh_fruit}", font_5, (255,255,255), 1470, 194)
                best_food = pygame.image.load(f"project-VAK/img/gui/food/best_food.png").convert_alpha()
                best_food = pygame.transform.scale(best_food, (int(best_food.get_width() * 2), int(best_food.get_height() * 2)))
                screen.blit(best_food, (1510, 159))
                draw_text(f"{big_fruit}", font_5, (255,255,255), 1520, 194)
        ...
        if game_is_stopped == True and counters_constat == -1:
                player.draw()
                for enemy in enemies_group:
                    enemy.draw()
                bullet_group.draw(screen)
                grenade_group.draw(screen)
                explosion_group.draw(screen)
                gradientRect( screen, (0, 0, 0), (0, 0, 0), pygame.Rect( 0,0, 2000, 1400 ), alpha=120)
                draw_text(f"PAUSE", font_9, (0, 0, 0), 640, 398)
                draw_text(f"PAUSE", font_9, (255, 255, 255), 640, 400)
        if not player.is_alive:
            gradientRect( screen, (0, 0, 0), (0, 0, 0), pygame.Rect( 0,0, 2000, 1400 ), alpha=190)
            pygame.mixer.music.stop()
            if diff_ch == 5:
                draw_text(f"due to the chosen difficulty, you lose all your", font, (135, 16, 16), 20, 800)
                draw_text(f"details and money. You knew what you were doing", font, (135, 16, 16), 20, 840)
                draw_text("GAME", font_8, (135, 16, 16), 470, 150)
                draw_text("OVER", font_8, (135, 16, 16), 510, 320)
                money = 0
                money_diamond = 0
                grass_cnt = 0
                best_det_cnt = 0
                ...
                draw_text("GAME", font_8, (255,255,255), 470, 150)
                draw_text("OVER", font_8, (255,255,255), 510, 320)
            if not antoher_add_flag:
                flag_game_over = True
            draw_text(f"press tab to exit", font, (0, 0, 0), 630, 600)
            draw_text(f"press tab to exit", font, (255, 255, 255), 630, 602)
        if flag_game_over == True:
            if diff_ch != 5:
                over_noise.play()
            else:
                grand_over_noise.play()
            flag_game_over = False
            antoher_add_flag = True
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                saving_game()
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_1 and player.is_alive:
                    if small_fruit >= 1 and player.max_health != player.health:
                        if small_fruit > 10 and skill_player_have[9] == 1:
                            if player.health + food_health_on_diff[diff_ch-1][3] > player.max_health:
                                player.health = player.max_health
                            else:
                                player.health += food_health_on_diff[diff_ch-1][3]
                        else:
                            if player.health + food_health_on_diff[diff_ch-1][0] > player.max_health:
                                player.health = player.max_health
                            else:
                                player.health += food_health_on_diff[diff_ch-1][0]
                        small_fruit -= 1
                        eat_s.play()
                    else:
                        cant_s.stop()
                        cant_s.play()
                if event.key == pygame.K_2 and player.is_alive:
                    if fresh_fruit >= 1 and player.max_health != player.health:
                        if player.health + food_health_on_diff[diff_ch-1][1] > player.max_health:
                            player.health = player.max_health
                        else:
                            player.health += food_health_on_diff[diff_ch-1][1]
                        fresh_fruit -= 1 
                        eat_s.play()
                    else:
                        cant_s.stop()
                        cant_s.play()  
                if event.key == pygame.K_3 and player.is_alive:
                    if big_fruit >= 1 and player.max_health != player.health:
                        if player.health + food_health_on_diff[diff_ch-1][2] > player.max_health:
                            player.health = player.max_health
                        else:
                            player.health += food_health_on_diff[diff_ch-1][2]
                        big_fruit -= 1
                        eat_s.play()
                    else:
                        cant_s.stop()
                ...
                if event.key == pygame.K_ESCAPE:
                    run = False
                    saving_game()
                if skill_player_have[5] == 1 and player.is_alive:
                    if event.key == pygame.K_q:
                        is_using_grenade = True
                if skill_player_have[16] == 1 and player.is_alive:
                    if event.key == pygame.K_v:
                        is_using_grenade_el = True
                
                if event.key == pygame.K_TAB:
                    if planet_cleared == True or player.is_alive is not True:
                        bullet_group.empty()
                        enemies_group.empty()
                        start_intro = True
                        shop_fade.fade_counter = 0
                        ...
                        city_noise.play()
                        random_track = [1,2,3,4]
                        player.second_breath_perk_used = False
                        random.shuffle(random_track)
                        for i in random_track:
                            main_music = pygame.mixer.music.load(f"project-VAK/img/sfx/main_menu/main_music{i}.mp3")
                        pygame.mixer.music.play(-1)
            ...
            if event.type == pygame.KEYUP:
                ...
                if event.key == pygame.K_d:
                    is_moving_right = False
                if skill_player_have[5] == 1:
                    if event.key == pygame.K_q and player.is_alive:
                        is_using_grenade = False
                        is_grenade_thrown = False
                if skill_player_have[16] == 1:
                    if event.key == pygame.K_v and player.is_alive:
                        is_using_grenade_el = False
                        is_grenade_thrown_el = False
                if event.key == pygame.K_w:
                    is_moving_up = False
    ...
    if current_window_selected == "credits":
        ...
        if mouse_pos[0] >= 820 and mouse_pos[0] <= 970:
            if mouse_pos[1] >= 408 and mouse_pos[1] <= 550:
                ...
                draw_text("mobs logic and spawning. Made the", font_3, (255,255,255), 20 , 750)
                draw_text("system of waves and difficulty", font_3, (255,255,255), 20 , 820)
        if back_button.draw(screen):
            current_window_selected = "main_menu"
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if mouse_pos[0] >= 820 and mouse_pos[0] <= 970:
                    if mouse_pos[1] >= 408 and mouse_pos[1] <= 550:
                        ...
                        maw.stop()
                        maw.play()
    pos = pygame.mouse.get_pos()  # update position 
